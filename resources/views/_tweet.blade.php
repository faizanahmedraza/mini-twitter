    ...
    <div>
        ...
        <div class="flex">
            <form action="/tweets/{{$tweet->id}}/like" method="POST">
                @csrf
                <div class="flex items-center mr-4 {{$tweet->isLikedBy(current_user()) ? 'text-blue-500 font-semibold' : 'text-gray-500'}}">
            ...
            <form action="/tweets/{{$tweet->id}}" method="POST">
                @csrf
                @method('DELETE')
                <div class="flex items-center {{$tweet->isDisLikedBy(current_user()) ? 'text-blue-500 font-semibold' : 'text-gray-500'}}">
                    <svg viewBox="0 0 20 20" class="w-3 mr-1">
                        <g id="Page-1" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
                            <g class="fill-current">
